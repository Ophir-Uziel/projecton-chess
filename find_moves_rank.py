    ...
    def __init__(self, chess_helper, net_dir_name = None):
        ...
        if net_dir_name:
            ...
            for angle in ["angle0", "angle1"]:
                tester_helper.make_dir(net_dir_name + "\\" +angle + "\\" + 'self_y_dir')
                tester_helper.make_dir(net_dir_name + "\\" +angle + "\\" + 'self_n_dir')
                tester_helper.make_dir(net_dir_name + "\\" +angle + "\\" + 'abv_y_dir')
                tester_helper.make_dir(net_dir_name + "\\" +angle + "\\" + 'abv_n_dir')
    """
    ...
    def get_move(self,sources_place, sources_self, sources_above,
        ...
        try:
            ...
            if self.net_dir_name:
                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) +"\\" +
                            'self_y_dir/im'+str(self.neuron_counter)+'.jpg', np.array(sources_self[real_idx_source]))
                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'abv_y_dir/im'+str(
                    self.neuron_counter)+'.jpg', np.array(sources_above[real_idx_source]))
                if len(sources_self) > 1:
                    ...
                    cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'self_n_dir/im' + str(self.neuron_counter) + '.jpg', np.array(sources_self[n_source_idx]))
                    cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'abv_n_dir/im' +
                                str(self.neuron_counter) + '.jpg', np.array(sources_above[n_source_idx]))

                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'self_y_dir/im' +
                            str(self.neuron_counter + 1) + '.jpg', np.array(targets_self[real_idx_target]))
                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" +'abv_y_dir/im' + str(
                    self.neuron_counter + 1) + '.jpg', np.array(targets_above[real_idx_target]))
                n_target_idx = real_idx_target
                while n_target_idx == real_idx_target:
                    n_target_idx = np.random.randint(0, len(targets_self))
                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'self_n_dir/im' +
                            str(self.neuron_counter + 1) + '.jpg', np.array(targets_self[n_target_idx]))
                cv2.imwrite(self.net_dir_name + "\\angle" + str(
                    angle_idx) + "\\" + 'abv_n_dir/im' +
                            str(self.neuron_counter + 1) + '.jpg', np.array(targets_above[n_target_idx]))
